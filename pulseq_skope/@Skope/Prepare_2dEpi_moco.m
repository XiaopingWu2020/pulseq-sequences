function Prepare_2dEpi_moco(this)
...
TE= this.seq_params.TE(1);
%nEchos= length(this.seq_params.TE);
...
fieldTrackingScheme= this.seq_params.fieldTrackingScheme;

myDelay= 0.5e-3; % a delay added in between tones and image readout and in between tones and field control delay and in between field control delay and spoiler to reduce the effects of preemphasis

fieldCtrlDuration= this.seq_params.fieldControlDuration; % duration during which background field is monitored. 
mr_fieldCtrlDuration= mr.makeDelay(fieldCtrlDuration);
...
% add gradient tones to spoilers for motion tracking
mr_gzSpoil=mr.makeTrapezoid('z',this.sys,'Area',deltak_x*Nx*4);% base gradient for all axes.
% if nEchos>1 && this.seq_params.maximizeTimeEfficiency
%     sf= 1;
%     basegrad.gx= mr_gzSpoil;
%     basegrad.gy= mr_gzSpoil;
%     basegrad.gz= mr_gzSpoil;
%     basegrad.gx.amplitude= 0;
%     basegrad.gy.amplitude= 0;
%     basegrad.gz.amplitude= sf.* basegrad.gz.amplitude;
% else
%     basegrad= [];
% end

% tone specs
tw= this.seq_params.gradToneDuration; %1e-3; %4.8e-3; % length of tones in second.
tw= this.Rasterize(tw, "grad");

f0= this.seq_params.toneFrequencies; % nominal tone frequencies [fx, fy, fz] in hz
A0= this.seq_params.toneAmplitude; % constant max tone amplitude for gx, gy, and gz in mT/m.
%[gtx, gty, gtz, trigDelay]= this.createGradientTones(A0,f0,tw,basegrad);
[gtx, gty, gtz]= this.createGradientTones(A0,f0,tw);
% todo: check gradient tones to ensure they do not violate slew rate
% limits

%
mr_gtx = mr.makeArbitraryGrad('x',(-1)* gtx,this.sys);
mr_gty = mr.makeArbitraryGrad('y',gty,this.sys);
mr_gtz = mr.makeArbitraryGrad('z',gtz,this.sys);

mr_trigMoco = mr.makeDigitalOutputPulse(this.seq_params.trigChannelMoco,...
    'duration', this.sys.gradRasterTime);
% if trigDelay- gradFreeDelay>= 0
%     mr_trigMoco.delay= trigDelay- gradFreeDelay;
% else
%     mr_gtx.delay= gradFreeDelay- trigDelay;
%     mr_gty.delay= gradFreeDelay- trigDelay;
%     mr_gtz.delay= gradFreeDelay- trigDelay;
% end

% Calculate timing
if Navigator>0
    minTE= mr_gz.fallTime + mr_gz.flatTime/2 + mr.calcDuration(mr_gzReph) ...
        + mr.calcDuration(mr_gxPre)+ Navigator*mr.calcDuration(mr_gx) ...
        + mr.calcDuration(mr_gyPre) + (Ny_pre+0.5)*mr.calcDuration(mr_gx);
else % no Navi
    minTE= mr_gz.fallTime + mr_gz.flatTime/2 + mr.calcDuration(mr_gzReph) ...
        + mr.calcDuration(mr_gxPre,mr_gyPre)+ (Ny_pre+0.5)*mr.calcDuration(mr_gx);
%     if nEchos==1
%         minTE= minTE+ + mr_mydelay.delay+ mr.calcDuration(mr_gtx,mr_gty,mr_gtz);
%     end
end

%
trig2AdcTime= [];
if contains(fieldTrackingScheme,"G","IgnoreCase",true)
    if Navigator>0
...
trig2toneTime=[]; trig2fieldCtrlTime=[];
switch fieldTrackingScheme
    case 'T'
        trig2toneTime= gradFreeDelay;
        skopeAcqDuration= trig2toneTime+ mr.calcDuration(mr_gtx,mr_gty,mr_gtz)+ 0.1e-3;
        this.seq_params.trigChannel= this.seq_params.trigChannelMoco;
        
    case 'TF'
        trig2toneTime= gradFreeDelay;
        trig2fieldCtrlTime= trig2toneTime+ mr.calcDuration(mr_gtx,mr_gty,mr_gtz)+ myDelay;
        skopeAcqDuration= trig2fieldCtrlTime+ mr_fieldCtrlDuration.delay+ 0.1e-3;
        this.seq_params.trigChannel= this.seq_params.trigChannelMoco;

    case 'GTF'
        minTE= minTE+ mr_gradFreeDelay.delay;
        trig2toneTime= (trig2AdcTime- mr_adc.delay) + Ny_meas*mr.calcDuration(mr_gx) + myDelay;
        trig2fieldCtrlTime= trig2toneTime+ mr.calcDuration(mr_gtx,mr_gty,mr_gtz)+ myDelay;
        skopeAcqDuration= trig2fieldCtrlTime+ mr_fieldCtrlDuration.delay+ 0.1e-3;

    case 'F'
        trig2fieldCtrlTime= gradFreeDelay;
        skopeAcqDuration= trig2fieldCtrlTime+ mr_fieldCtrlDuration.delay+ 0.1e-3;
    ...
    case 'G'
        minTE= minTE+ mr_gradFreeDelay.delay;
        skopeAcqDuration= (trig2AdcTime- mr_adc.delay) + Ny_meas*mr.calcDuration(mr_gx) + 0.1e-3;

    otherwise
        error('-> Field tracking scheme specified is NOT supported...')
end
...
minExcitTR= mr.calcDuration(mr_rfFatSat)+ mr.calcDuration(mr_gzFatSat) ...
    ...
    + mr_fieldCtrlDuration.delay+ 3*myDelay+ mr.calcDuration(mr_gtx,mr_gty,mr_gtz)+ mr.calcDuration(mr_gzSpoil);
...
if ~isempty(trig2toneTime)
    this.seq_params.trigger2toneTime= trig2toneTime;
end
if ~isempty(trig2fieldCtrlTime)
    this.seq_params.trigger2fieldCtrlTime= trig2fieldCtrlTime;
end
% motion tracking
this.seq_params.skopeAcqDurationMoco= gradFreeDelay+ mr.calcDuration(mr_gtx,mr_gty,mr_gtz)+ 0.1e-3;
...
this.seq_params.echoSpacing= mr.calcDuration(mr_gx);
this.seq_params.echoTrainLength = Ny_meas;
this.seq_params.prePhaserDuration= mr.calcDuration(mr_gxPre,mr_gyPre);
...
for r=1:Nreps
    ...
    this.seq.addBlock(mr.makeLabel('SET','SLC', 0), mr.makeLabel('SET','RO',0));
    for s=1:Nslices
        ...
        if Navigator>0
            this.seq.addBlock(mr_gxPre, mr.makeLabel('SET','RO',1));
            for n=1:Navigator
                this.seq.addBlock(mr_gx,mr_adc, ...
            ...
            this.seq.addBlock(mr_gxPre, mr_gyPre, ...
                ...
                mr.makeLabel('SET','RO',1));% lin/nav/avg reset
        end
        ...
        for i=1:Ny_meas
            ...
            if i==1
                this.seq.addBlock(mr_gx,mr_gyBlipup,mr_adc); % Read the first line of k-space with a single half-blip at the end
            elseif i==Ny_meas
                this.seq.addBlock(mr_gx,mr_gyBlipdown,mr_adc,mr.makeLabel('SET','RO',0)); % Read the last line of k-space with a single half-blip at the beginning
            else
                this.seq.addBlock(mr_gx,mr_gyBlipdownup,mr_adc); % Read an intermediate line of k-space with a half-blip at the beginning and a half-blip at the end
            end
        ...
        % motion tracking
        this.seq.addBlock(mr.makeDelay(myDelay- mr_gradFreeDelay.delay));
        this.seq.addBlock(mr_trigMoco, mr_gradFreeDelay);

        this.seq.addBlock(mr_gtx, mr_gty, mr_gtz, mr.makeLabel('SET','GT',1));

        if fieldTrackingScheme== "F"
            this.seq.addBlock(mr.makeDelay(myDelay- mr_gradFreeDelay.delay));
            this.seq.addBlock(mr_trig, mr_gradFreeDelay);
        else
            this.seq.addBlock(mr.makeDelay(myDelay));
        end

        this.seq.addBlock(mr_fieldCtrlDuration);
        this.seq.addBlock(mr.makeDelay(myDelay));
