function Prepare_2dSpiral_moco(this)
...
image2gradToneDelay= this.seq_params.image2gradToneDelay; % a delay added in between tones and imaging gradients to reduce the effects of preemphasis
% if nEchos== 1
%     mydelay= mydelay- gradFreeDelay;
% end
mr_mydelay= mr.makeDelay(image2gradToneDelay);
...
this.seq_params.adcLength= mr_adc.numSamples;
...
% spoilers
dur= this.Rasterize(max(abs(spiral_grad_shape(:,end))./ (0.9*this.sys.maxSlew)),'grad');
mr_gxSpoil=mr.makeExtendedTrapezoid('x','times',[0 dur],'amplitudes',(-1)*[spiral_grad_shape(1,end),0]); %todo: make a really good spoiler
mr_gySpoil=mr.makeExtendedTrapezoid('y','times',[0 dur],'amplitudes',[spiral_grad_shape(2,end),0]); %todo: make a really good spoiler
...
if nEchos==1
    minTE= minTE+ image2gradToneDelay+ mr.calcDuration(mr_gtx,mr_gty,mr_gtz);
end
if TE< minTE
    TE= minTE;
...
if nEchos>1
    minExcitTR= minExcitTR+ mr.calcDuration(mr_gxSpoil,mr_gySpoil)+ image2gradToneDelay+ mr.calcDuration(mr_gtx,mr_gty,mr_gtz);
    if ~this.seq_params.maximizeTimeEfficiency
        minExcitTR= minExcitTR+ mr.calcDuration(mr_gzSpoil);
    end
else
    minExcitTR= minExcitTR+ mr.calcDuration(mr_gxSpoil,mr_gySpoil,mr_gzSpoil);
end
...
switch fieldTrackingScheme
    case 'T'
        skopeAcqDuration= gradFreeDelay+ tw+ 0.1e-3;
        this.seq_params.trigChannel= this.seq_params.trigChannelMoco;
    case 'TF'
        skopeAcqDuration= gradFreeDelay+ tw+ image2gradToneDelay+ 0.1e-3;
        this.seq_params.trigChannel= this.seq_params.trigChannelMoco;
        this.seq_params.trigger2fieldCtrlTime= gradFreeDelay+ tw;
    case 'TFG'
        skopeAcqDuration= gradFreeDelay+ tw+ image2gradToneDelay+ mr.calcDuration(mr_gx,mr_gy,mr_adc) + 0.1e-3;
        this.seq_params.trigChannel= this.seq_params.trigChannelMoco;
        trig2AdcTime= gradFreeDelay+ tw+ image2gradToneDelay;
        this.seq_params.trigger2fieldCtrlTime= gradFreeDelay+ tw;
    case 'F'
        skopeAcqDuration= image2gradToneDelay+ 0.1e-3;
        this.seq_params.trigger2fieldCtrlTime= 0;
    case 'FG'
        skopeAcqDuration= image2gradToneDelay+ mr.calcDuration(mr_gx,mr_gy,mr_adc) + 0.1e-3;
        trig2AdcTime= image2gradToneDelay;
        this.seq_params.trigger2fieldCtrlTime= 0;
    case 'G'
        skopeAcqDuration= gradFreeDelay + mr.calcDuration(mr_gx,mr_gy,mr_adc) + 0.1e-3;
        mr_trig.delay= image2gradToneDelay - gradFreeDelay;
...
if ~isempty(trig2AdcTime)
    this.seq_params.trigger2AdcTime= trig2AdcTime;
     ...
     if this.seq_params.nPrescans>0
        ...
        disp('-> Prescan is not supported in the field tracking scheme chosen...')
        disp('-> Number of prescans is reset to 0...')
    end
end
...
% motion tracking
this.seq_params.skopeAcqDurationMoco= gradFreeDelay+ tw+ 0.1e-3;
...
if this.seq_params.nPrescans>0
    ...
    if this.seq_params.useSingleAdcSegment4Sync
        ...
        for i=1: this.seq_params.nPrescans
            % insert trigger
            this.seq.addBlock(mr_trigMoco, mr_gtx, mr_gty, mr_gtz); %

            this.seq.addBlock(mr_trig, mr_mydelay);
            %
...
% 
% % limit analysis
% safety_magrin= 0.8; % we need that  otherwise we just about violate the slew rate due to the rounding errors
